...
public abstract class TypeUtils {
    ...
    private static boolean isAssignable(Type type, Class<?> toClass) {
        ...
        if (type instanceof Class<?>) {
            ...
            return isAssignable((Class<?>) type, toClass, true);
        }
    ...
    private static boolean isAssignable(Type type, ParameterizedType toParameterizedType,
        ...
        // load the target type's raw type
        Class<?> toClass = getRawType(toParameterizedType);
        // load the subject type's type arguments including owner type arguments
        ...
        // load the target type's type arguments including owner type arguments
    ...
    private static Map<TypeVariable<?>, Type> getTypeArguments(
        ...
        if (ownerType instanceof ParameterizedType) {
            // load the owner type arguments first
        ...
        // load the subject parameterized type's arguments
        Type[] typeArgs = parameterizedType.getActualTypeArguments();
        // and load the corresponding type variables from the raw class
     ...
     * {@link Iterable}. Since <code>TreeSet</code>'s parameter maps
    ...
    public static Map<TypeVariable<?>, Type> determineTypeArguments(Class<?> cls,
        ...
        // load the next class in the inheritance hierarchy
        ...
        // load the type variables of the mid class that map to the type
    ...
    private static Type getClosestParentType(Class<?> cls, Class<?> superClass) {
        ...
        if (superClass.isInterface()) {
            // load the generic interfaces of the subject class
    ...
    public static Class<?> getRawType(Type type, Type assigningType) {
        ...
        if (type instanceof ParameterizedType) {
            // simple enough to load the raw type of a ParameterizedType
        ...
        if (type instanceof TypeVariable<?>) {
            ...
            // load the entity declaring this type variable
            Object genericDeclaration = ((TypeVariable<?>) type).getGenericDeclaration();

            // can't load the raw type of a method- or constructor-declared type
            // variable
            if (!(genericDeclaration instanceof Class<?>)) {
                return null;
            }

            // load the type arguments for the declaring class/interface based
            ...
            // load the argument assigned to this type variable
            ...
            // load the argument for this type variable
            return getRawType(typeArgument, assigningType);
        }

        if (type instanceof GenericArrayType) {
            // load raw component type
     ...
     * @return
     */
    public static Class<?> resolveActualType(Class<?> subclass, Class<?> superClass, int index){
        Class<?> clazz = null;
        for(Type type : subclass.getGenericInterfaces()){
//			if(getRawType(type) == superClass){
            clazz = getRawType(((ParameterizedType)type).getActualTypeArguments()[index]);
            break;
//			}
        }
        return clazz;
    ...
    public static boolean isNumberType(Class<?> targetClazz) {
        ...
        // 判断原始类,过滤掉特殊的基本类型
        if (targetClazz == boolean.class || targetClazz == void.class) {
            return false;
        }
    ...
    /**
     * 解析出指定的注解的字段，包含父类（但不包含Object.class）
     * @param annotationType
     * @param classType
     * @return
     */
    public static <T extends Annotation> List<Field> resolveAnnotationField(Class<T> annotationType, Class<?> classType){
        if(annotationType == null || classType == null){
        ...
        if(superClass != Object.class){
            fields.addAll(resolveAnnotationField(annotationType, superClass));
        }
     ...
     * @return
     */
    public static Type[] resolveTypeArguments(Class<?> subclass, Class<?> superClass){
        TypeToken<?> child = TypeToken.of(subclass);
        TypeVariable<?>[] typeParameters = superClass.getTypeParameters();
        Type[] types = new Type[typeParameters.length];
        for(int i = 0; i < typeParameters.length; i++){
            TypeToken<?> t = child.resolveType(typeParameters[i]);
            types[i] = t.getType();
        }
        return types;
    }
     ...
     * @return
     */
    public static Class<?>[] resolveClassArguments(Class<?> subclass, Class<?> superClass){
        TypeToken<?> child = TypeToken.of(subclass);
        TypeVariable<?>[] typeParameters = superClass.getTypeParameters();
        Class<?>[] types = new Class<?>[typeParameters.length];
        for(int i = 0; i < typeParameters.length; i++){
            TypeToken<?> t = child.resolveType(typeParameters[i]);
            types[i] = t.getRawType();
        }
        return types;
    }
     ...
     * @return
     */
    public static <T extends Annotation> List<Method> resolveAnnotationMethods(Class<T> annotationType, Class<?> classType){
        if(annotationType == null || classType == null){
            return java.util.Collections.emptyList();
        }
        ...
        Arrays.asList(classType.getDeclaredMethods()).forEach(method->{
     ...
     * @return
     */
    public static <T extends Annotation> T resolveAnnotation(Class<T> annotationType, Class<?> classType){
        if(annotationType == null || classType == null){
            return null;
        }
        ...
        if(annotation == null){
            Class<?> superClass = classType.getSuperclass();
            if(superClass != Object.class){
                annotation = resolveAnnotation(annotationType, superClass);
            }
        }

        if (annotation == null){
            Class<?>[] interfaces = classType.getInterfaces();
            for(Class<?> interfactType : interfaces){
                annotation = resolveAnnotation(annotationType,interfactType);
                if(annotation != null)
                    break;
            }
        }
     ...
     * @return
     */
    public static boolean isPrimitiveOrWrapperOrString(Class<?> elementType) {
        return TypeUtils.isPrimitiveOrWrapper(elementType) || elementType == String.class;
    }

    /**
     * 获取subclass或subclass的父类中的父类或实现的接口类型是parentClass 的类
     * @param subclass
     * @param parentClass
     * @return
     */
    public static Class<?> getSubclassByParentClass(Class<?> subclass, Class<?> parentClass) {
        ...
        if(superClass != null && superClass != Object.class){
            targetClass = getSubclassByParentClass(superClass, parentClass);
        }

        if(targetClass == null){
            Class<?>[] interfaces = subclass.getInterfaces();
            for(Class<?> interfactType : interfaces){
                if(interfactType == parentClass){
                    targetClass = subclass;
                } else {
                    targetClass = getSubclassByParentClass(interfactType, parentClass);
                    if(targetClass != null){
                        break;
                    }
                }
