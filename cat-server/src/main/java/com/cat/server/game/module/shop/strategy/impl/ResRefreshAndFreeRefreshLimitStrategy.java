...
/**
 * 限制资源刷新次数(3次),免费刷新次数(3次)
 * @author Jeremy
 */
public class ResRefreshAndFreeRefreshLimitStrategy extends AbstractRefreshStrategy{
	
	public ResRefreshAndFreeRefreshLimitStrategy(AbstractShopType<? extends IConfigShop> shopType) {
		super(shopType);
	}
	...
	public ErrorCode checkRefresh(ConfigShopControl config, ShopDomain domain) {
		...
		if (shop.getFreeRefreshNum() + 1 < config.getFreeRefreshNum()) {
			...
			if (!resourceGroupService.checkEnought(domain.getId(), config.getRefreshCost())) {
				//次数足够, 但是消耗不足
