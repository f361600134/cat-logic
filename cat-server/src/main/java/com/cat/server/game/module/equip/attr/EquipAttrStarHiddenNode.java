 ...
 */
public class EquipAttrStarHiddenNode extends AbstractEquipAttrNode{

	public EquipAttrStarHiddenNode(Equip equip) {
		super(equip);
	}
    ...
    protected AttributeDictionary calculateAttrDic() {
        ...
        final int starHiddenLv = equip.getStarHiddenLevel();
        if (starHiddenLv == 0) {
        	//如果是0级
			return AttributeUtils.EMPTY;
		}
        //下标=隐藏等级-1
        Map<Integer, Integer> attr = config.getStarHiddenAttr(starHiddenLv-1);
        if (attr == null) {
        	return AttributeUtils.EMPTY; 
		}
