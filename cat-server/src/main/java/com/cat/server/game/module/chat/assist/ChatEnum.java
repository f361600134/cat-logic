...
//			PlayerService service =  SpringContextHolder.getBean(PlayerService.class);
...
//			}
		...
		public ChatDomain getDomain(Long playerId) {
			//TODO 获取玩家所在省id
			int provinceId = 0;
			ChatService service = SpringContextHolder.getBean(ChatService.class);
	...
	CH_SYSTEM(6) {// 系统频道
		...
		@Override
		public Collection<Long> findPlayerIds(ChatDomain domain) {
			PlayerService playerService =  SpringContextHolder.getBean(PlayerService.class);
			return  playerService.getPlayerIds();
		}
	...
	public int check(Player player, Chat chat) {
		...
		ConfigChat config = configManager.getConfig(ConfigChat.class, getCh());
		if (config == null) {
