package com.cat.api.core.task.impl;
...
import com.cat.api.core.task.DefaultTaskHandle;
import com.cat.api.core.task.Task;
import com.cat.api.core.task.TaskHandle;
import com.cat.api.core.task.TaskRunner;
import com.cat.api.core.task.ThreadFactoryImpl;

/**
 * 公共任务线程池, 默认情况下1条线程执行<br>
 * 可以给定线程池内线程数量, 但是这样同一个任务不确定线程执行情况
 * @author Jeremy
 */
public class CommonTaskExecutor {
    ...
    public TaskHandle scheduleTaskStartAtNextTimeUnit(Task task, long period, TimeUnit delayUnit) {
        ...
        ScheduledFuture<?> scheduleTask = executor.scheduleAtFixedRate(taskRunner, delayTime, delayUnit.toMillis(period), TimeUnit.MILLISECONDS);
        DefaultTaskHandle taskHandle = new DefaultTaskHandle(scheduleTask);
        return taskHandle;
    }
